...
class DatabaseManager:
    ...
    def get_random_prize(self):
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()  # типа курсор
            cur.execute('SELECT prize_id, image FROM prizes WHERE used = 0')
            prizes = cur.fetchall()
            if prizes:
                return random.choice(prizes)  # типа рандом приз
            return None  # если их ет
...
if __name__ == '__main__':
    ...
    print("Prize image for ID 1:", manager.get_prize_img(1))
    print("Random prize:", manager.get_random_prize())